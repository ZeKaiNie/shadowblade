...
> ⚠️ **未核实警告**：本文件大量条目（攻击事件规模、竞品数据、arXiv 编号、F1/检出率、
> 数据集样本量、IOC 等）源自早期 AI 辅助整理，**尚未逐条核实，极可能存在幻觉或过时**。
> 正式写入论文/申报书前，**必须**到 arXiv / DBLP / Google Scholar / 官方来源逐条查证；
> 查不到出处的一律不得引用。此处内容仅作"待查素材"，不得当作事实。
