...
- **下一步（待用户确认，真正能提召回的杠杆）**：① **可疑外传目标信誉**（网络即便已声明，外联 discord CDN/telegram/rentry/pastebin/硬编码 IP 仍作强信号，预期打掉那 ~10 个"声明广权限但外传恶意目标"的样本、且良性不碰这些目标→不牺牲 FPR）；② 样本可执行性（Windows/缺库样本单独归静态/LLM 路径）；③ 更大批 + 消融 + 跨数据集泛化；④ 方向 B 测试台；⑤ 论文初稿 + arXiv preprint。
- **PR 现状**：#4/#5/#6/#7/#8 均已合入 main。
