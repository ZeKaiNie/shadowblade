...
- **下一步（待用户确认）**：① 扩大/更新 IOC 与目标信誉源（接公开威胁情报，别只靠硬编码清单）；② 样本可执行性（Windows/缺库/语法非法样本单独归静态/LLM 路径，或提供贴近其预期环境的执行路径）；③ 更大批 + 消融（量化诱饵/入口触发/目标信誉各自边际贡献）+ 跨数据集泛化；④ 方向 B 测试台；⑤ 论文初稿 + arXiv preprint。
- **PR 现状**：#4–#10 均已合入 main（#9 入口触发、#10 交接书）。
