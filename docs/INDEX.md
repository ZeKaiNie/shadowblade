...
- **⭐ 当前研究主线（锁定 A+B，同一套 canary 方法打两个面）**：
  ...
  - **当前进度**：A 的数据/评测/一致性核验/动态沙箱已通，真实 50/50 小批 = 高精度零误报低召回确认层
    （P=1.00/FPR=0.00/Recall=0.16）。诱饵扩展(v2)+入口触发(v3)已做但未提升该批召回（瓶颈：样本在通用沙箱跑不起来 + 能力粒度盲区）。
    **下一步：可疑外传目标信誉（声明了网络也拦恶意目标）**。
...
- ✅ 诱饵扩展(v2，~20 诱饵+读凭据信号)、入口触发(v3，调用样本定义的入口函数) 已做；机制经合成/单测验证正确，但**同批召回未变**（诊断：50 恶意中 7 语法错/16 运行崩溃/18 有敏感行为、其中 10 如实声明→放行）。
- ⬜ 可疑目标信誉、下载-执行链、样本可执行性、方向 B 测试台、消融、跨数据集泛化、论文初稿 未做。
