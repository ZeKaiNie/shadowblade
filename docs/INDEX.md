...
> 协作规则见根目录 `AGENTS.md`。
> 最后更新：2026-07-18（门面/记忆文件同步到当前 A+B 主线：README/PROJECT_CONTEXT/快速上下文/progress 横幅）
...
- **⭐ 当前研究主线（锁定 A+B，同一套 canary 方法打两个面）**：
  ...
  - **当前进度**：A 的数据/评测/一致性核验/动态沙箱已通，真实 50/50 小批 = 高精度零误报低召回确认层
    （P=1.00/FPR=0.00/Recall=0.16）。**下一步：诱饵扩展提召回**。
...
- ⬜ 诱饵扩展、可疑目标信誉、下载-执行链、方向 B 测试台、消融、跨数据集泛化、论文初稿 未做。
